...
func GetTopUpInfo(c *gin.Context) {
	...
	promotionPreviews, promotionPreviewErr := model.GetRechargePromotionPreviews()
	if promotionPreviewErr != nil {
		common.ApiError(c, promotionPreviewErr)
		return
	}
	...
	data := gin.H{
		...
		"recharge_promotions":     promotionPreviews,
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		result, completeErr := model.CompleteTopUp(verifyInfo.ServiceTradeNo, model.TopUpCompleteOptions{
			ExpectedPaymentProvider: model.PaymentProviderEpay,
			PaymentMethod:           verifyInfo.Type,
			QuotaCalculator: func(order *model.TopUp) (int, error) {
				quota, clamp := common.QuotaFromDecimalChecked(decimal.NewFromInt(order.Amount).Mul(decimal.NewFromFloat(common.QuotaPerUnit)))
				if clamp != nil {
					return 0, clamp
				}
				if quota <= 0 {
		...
		if !result.AlreadyCompleted {
			logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值成功 trade_no=%s user_id=%d client_ip=%s quota_to_add=%d money=%.2f", result.TopUp.TradeNo, result.TopUp.UserId, c.ClientIP(), result.QuotaAdded, result.TopUp.Money))
			model.RecordTopupLog(result.TopUp.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(result.QuotaAdded), result.TopUp.Money), c.ClientIP(), result.TopUp.PaymentMethod, "epay")
		}
	} else {
